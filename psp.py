...
class MypSp(nn.Module):
	...
	def __init__(self, opts):
		...
		# Define architecture
		self.encoder = psp_encoders.BackboneEncoderUsingLastLayerIntoWPlus(50, 'ir_se', self.opts)
		self.decoder = Generator(self.opts.output_size, 512, 8)
		self.mapping_net = psp_encoders.MappingNetowrk(self.opts)
	...
	def load_weights(self):
		if self.opts.checkpoint_path is not None:
			...
			self.mapping_net.load_state_dict(get_keys(ckpt, 'mapping_net'), strict=True)
			self.decoder.load_state_dict(get_keys(ckpt, 'decoder'), strict=True)
			self.__load_latent_avg(ckpt)
	def forward(self, x, restyle_latent, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
	            inject_latent=None, return_latents=False, alpha=None,return_code=False):
		
		if input_code:
			encoder_codes = x
		else:
			encoder_codes = self.encoder(x)
		
		input_is_latent = True
		cat = torch.cat((torch.flatten(restyle_latent,start_dim=1),torch.flatten(encoder_codes,start_dim=1)),dim=1)
		codes = torch.reshape(self.mapping_net.forward(cat),[cat.size()[0],18,512])
		...
		if return_code and return_latents:
			return images, result_latent, encoder_codes
		if return_latents:
...
class RestylepSp(nn.Module):
    ...
    def __init__(self, opts):
        super(RestylepSp, self).__init__()
        self.set_opts(opts)
    ...
    def set_encoder(self):
        if self.opts.encoder_type == 'GradualStyleEncoder':
            encoder = psp_encoders.GradualStyleEncoder(50, 'ir_se', self.n_styles, self.opts)
        elif self.opts.encoder_type == 'ResNetGradualStyleEncoder':
            encoder = psp_encoders.ResNetGradualStyleEncoder(self.n_styles, self.opts)
        elif self.opts.encoder_type == 'BackboneEncoder':
            encoder = psp_encoders.BackboneEncoder(50, 'ir_se', self.n_styles, self.opts)
        elif self.opts.encoder_type == 'ResNetBackboneEncoder':
            encoder = psp_encoders.ResNetBackboneEncoder(self.n_styles, self.opts)
        else:
